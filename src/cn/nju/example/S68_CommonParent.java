...
/**
 * 注：
 * 本题解参考 Github 上 nibnait 的题解而来 （这题写测试用例实在太累了 <-_-> ）
 * 修改了原先 getNodePath 方法中采用 void 返回类型会错误删除路径节点的问题
 */
...
public class S68_CommonParent {
     ...
     * @param path
     */
    private static boolean getNodePath(TreeNode root, TreeNode p, List<TreeNode> path) {
        if (root == null){
            return false;
        }
        path.add(root);
